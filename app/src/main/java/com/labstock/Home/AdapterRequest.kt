...
class AdapterRequest(private val items: List<ItemRequest>) :
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.tvStatus.setTextColor(ContextCompat.getColor(holder.itemView.context, item.status.textColor));
        drawable.setColor(ContextCompat.getColor(holder.itemView.context, item.status.bgColor))

        fun Int.toPx(): Int = (this * holder.itemView.context.resources.displayMetrics.density).toInt()
